...
$k = 7$
```{r BoneMarrow-structure-plot-k7, fig.width=8, fig.height=2.5, message=FALSE}
...
samples$cell_group[samples$cell_group %in% names(which(table(samples$cell_group) < 10))] <- "others"
table(samples$cell_group)

samples$cell_group <- factor(samples$cell_group, levels = c("Hematopoietic progenitors", 
                                                            "Erythroblasts",
                                                            "B cells", "Immature B cells","Activated B cells", 
                                                            "T cells", "Regulatory T cells", "NK cells","Dendritic cells", 
                                                            "Monocytes","Macrophages"))

p.structure <- structure_plot(select(fit_multinom,loadings = idx_selected_samples),
                              grouping = samples[idx_selected_samples, "cell_group"],
                              n = Inf,gap = 40, perplexity = 50,
                              topics = 1:7,colors = colors_topics,
...
p.structure <- structure_plot(select(fit_multinom,loadings = idx_selected_samples),
                              grouping = samples[idx_selected_samples, "cell_group"],
                              n = Inf,gap = 40, perplexity = 50,
                              topics = 1:8,colors = colors_topics,
                              num_threads = 6,verbose = FALSE)
