---
title: "Process data for Buenrostro *et al* (2018) scATAC-seq dataset with different options"
author: "Kaixuan Luo"
...
The ATAC-seq peaks were defined using the bulk ATAC-seq samples. (This may not be ideal 
as this approach may miss cell-type specific peaks that do not pass the peak calling threshold 
using all the bulk ATAC-seq samples. )
...
```{r option2-plot-distribution, include=FALSE}
y <- table(summary(counts)$x)
x <- names(y)
y <- as.numeric(y)
plot(x,y,pch = 20,log = "y",cex = 0.65,
...
Save processed data: `/project2/mstephens/kevinluo/scATACseq-topics/data/Buenrostro_2018/processed_data_Chen2019pipeline/`
```{r option2-save-data, eval=FALSE}
...
samples metadata
```{r option3-metadata, eval=FALSE}
samples <- read.table('/project2/mstephens/kevinluo/scATACseq-topics/data/Buenrostro_2018/data/input_Chen_2019/metadata.tsv', header = TRUE, stringsAsFactors=FALSE, quote="",row.names=1)
```

peaks (from aggregated scATAC-seq)
```{r option3-peaks, eval=FALSE}
...
```{r, eval=FALSE}
fragment_counts <- getCounts(file.path(dir_bamfiles,bamfiles), 
                             peaks, 
                             paired = TRUE, 
                             by_rg = TRUE, 
                             format = "bam", 
...
```{r plot-distribution-4, include=FALSE}
y <- table(summary(counts)$x)
x <- names(y)
y <- as.numeric(y)
plot(x,y,pch = 20,log = "y",cex = 0.65,
...
Save processed data: `/project2/mstephens/kevinluo/scATACseq-topics/data/Buenrostro_2018/processed_data_Chen2019pipeline/chromVAR/`
```{r, eval=FALSE}
save(list = c("samples", "peaks", "counts"), 
...
```{r plot-distribution-5, include=FALSE}
y <- table(summary(counts)$x)
x <- names(y)
y <- as.numeric(y)
plot(x,y,pch = 20,log = "y",cex = 0.65,
