...
n <- length(motifs)
homer_pvals <- matrix(0,n,k)
rownames(homer_pvals) <- motifs
colnames(homer_pvals) <- topics
for (i in topics) {
  dat  <- homer[[i]]
  rows <- which(!duplicated(dat[,"Motif Name"]))
  dat  <- dat[rows,]
  rownames(dat)   <- dat[,"Motif Name"]
  homer_pvals[,i] <- dat[motifs,"P-value"]
}
dat  <- homer$k1
rows <- which(!duplicated(dat[,"Motif Name"]))
dat  <- dat[rows,]
rownames(dat) <- dat[,"Motif Name"]
homer_pvals <- cbind(dat[motifs,c("Motif Name","Consensus")],homer_pvals)
```

```{r write-homer-results, echo=FALSE}
write.csv(format(homer_pvals,scientific = TRUE,digits = 2),
          "homer_buenrostro.csv",row.names = FALSE)
```
