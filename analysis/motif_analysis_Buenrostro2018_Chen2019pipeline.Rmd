...
## Focus on myeloid differentiation (HSC, CMP, GMP, Momo)
...
```{r region-volcano-plot, fig.width=5, fig.height=3}
...
Heatmap of motif enrichment -log10(p-value). 
Order motifs by hierarchical clustering.
```{r selected-motifs-heatmap-pvalue, fig.width=8, fig.height=6}
...
Heatmap of motif enrichment z-score.
Order motifs by hierarchical clustering.
```{r selected-motifs-heatmap-zscore, fig.width=8, fig.height=6}
...
```{r plot-GATA-logos, fig.width=5, fig.height=1.5,  message=FALSE, warning=FALSE}
...
```{r plot-CEBP-logos, fig.width=5, fig.height=1.5, message=FALSE, warning=FALSE}
# Plot GATA motifs in topic 4
k = 1
motif_order <- order(motif_res$mlog10P[,k], decreasing = T)
motifs <- rownames(motif_res$motifs[motif_order,])
