...
Here we perform TF motif enrichment analysis
...
```{r compile-homer-motif-results-lsfr0.01}
...
```{r all-motif-enrichment-res-lsfr0.01, eval=FALSE, include=FALSE}
...
```{r count-shared-regions, include=FALSE, eval=FALSE}
# Regions shared between topics
sprintf("%d regions shared between topic 1 and 2.\n", length(intersect(selected_regions$k1$name, selected_regions$k2$name)))
sprintf("%d regions shared between topic 3 and 4.\n", length(intersect(selected_regions$k3$name, selected_regions$k4$name)))
sprintf("%d regions shared between topic 5 and 6.\n", length(intersect(selected_regions$k5$name, selected_regions$k6$name)))
sprintf("%d regions shared between topic 7 and 8.\n", length(intersect(selected_regions$k7$name, selected_regions$k8$name)))
```

List top 10 motifs for each topic
```{r top-motifs-lsfr0.01}
...
```{r motif-heatmap-pvalue-lsfr0.01, fig.width=7, fig.height=12}
...
```{r motif-enrichment-ranking-plot-lsfr0.01, fig.width=10, fig.height=7, warning=FALSE}
...
```{r count-shared-regions-2, include=FALSE, eval=FALSE}
# Regions shared between topics
sprintf("%d regions shared between topic 1 and 2.\n", length(intersect(selected_regions$k1$name, selected_regions$k2$name)))
sprintf("%d regions shared between topic 3 and 4.\n", length(intersect(selected_regions$k3$name, selected_regions$k4$name)))
sprintf("%d regions shared between topic 5 and 6.\n", length(intersect(selected_regions$k5$name, selected_regions$k6$name)))
