...
#### save data for `resting` samples
```{r}
data.dir <- "/project2/mstephens/kevinluo/scATACseq-topics/data/Lareau_2019/bone_marrow/processed_data/"
load(file.path(data.dir, "Lareau_2019_bonemarrow.RData"))

idx_resting <- which(samples$Condition == "Resting")
samples <- samples[idx_resting,]
counts <- counts[idx_resting,]

saveRDS(counts, file.path(data.dir, "counts_Lareau_2019_bonemarrow_resting.rds"))

save(list = c("samples", "peaks", "counts"), file = file.path(data.dir, "Lareau_2019_bonemarrow_resting.RData"))
...
cat(sprintf("Number of samples (cells) that are resting: %d\n",nrow(counts)))
cat(sprintf("Number of peaks: %d\n",ncol(counts)))
cat(sprintf("Proportion of counts that are non-zero: %0.1f%%.\n",
            100*mean(counts > 0)))
...
#### save data for `stimulated` samples
```{r}
data.dir <- "/project2/mstephens/kevinluo/scATACseq-topics/data/Lareau_2019/bone_marrow/processed_data/"
load(file.path(data.dir, "Lareau_2019_bonemarrow.RData"))

idx_stimulated <- which(samples$Condition == "Stimulated")
samples <- samples[idx_stimulated,]
counts <- counts[idx_stimulated,]

saveRDS(counts, file.path(data.dir, "counts_Lareau_2019_bonemarrow_stimulated.rds"))

save(list = c("samples", "peaks", "counts"), file = file.path(data.dir, "Lareau_2019_bonemarrow_stimulated.RData"))
...
cat(sprintf("Number of samples (cells) that are stimulated: %d\n",nrow(counts)))
cat(sprintf("Number of peaks: %d\n",ncol(counts)))
cat(sprintf("Proportion of counts that are non-zero: %0.1f%%.\n",
            100*mean(counts > 0)))
