---
title: "Clustering Buenrostro *et al* (2018) scATAC-seq data using topic proportions with K = 14"
author: Kaixuan Luo
output:
...
multinomial topic model with $k = 14$.
...
p.pca3.5 <- labeled_pca_plot(fit,9:10,samples$label,font_size = 7,
                       legend_label = "Cell labels")
p.pca3.6 <- labeled_pca_plot(fit,11:12,samples$label,font_size = 7,
                       legend_label = "Cell labels")
...
p.structure.1 <- structure_plot(poisson2multinom(fit),
                     ...
                     perplexity = 50,topics = 1:14,colors = colors_topics,
...
Define clusters using k-means with $k = 25$:
```{r kmeans-unbinarized-3}
set.seed(10)

clusters.25 <- factor(kmeans(poisson2multinom(fit)$L,centers = 25)$cluster)
print(sort(table(clusters.25),decreasing = TRUE))
```

Structure plot based on k-means clusters
```{r structure-plot-kmeans-unbinarized-3, fig.width=7, fig.height=3}
p.structure.4 <- structure_plot(poisson2multinom(fit),
                     grouping = clusters.25,
                     rows = order(samples$label), # samples are grouped by clusters first, then by celltype labels
                     n = Inf,gap = 20,
                     perplexity = 50,topics = 1:14, colors = colors_topics,
                     num_threads = 4,verbose = FALSE)
...
### Refine clusters
We then further refine the clusters based on k-means result with $k = 25$:
merge clusters 3, 4;
merge clusters 5, 8, 10, 11, 20;
merge clusters 15 and 18;
merge clusters 6 and 19;
merge clusters 14 and 22.

```{r refine-cluster-unbinarized-1}
clusters.merged <- clusters.25
clusters.merged[clusters.25 %in% c(3,4)] <- 3
clusters.merged[clusters.25 %in% c(5,8,10,11,20)] <- 5
clusters.merged[clusters.25 %in% c(15,18)] <- 15
clusters.merged[clusters.25 %in% c(6,19)] <- 6
clusters.merged[clusters.25 %in% c(14,22)] <- 14
