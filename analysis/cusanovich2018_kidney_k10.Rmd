...
p1 <- structure_plot(fit_binom,grouping = samples$cell_label,gap = 20,
                     verbose = FALSE,colors = topic_colors,
					 topics = c(6,5,2,10,1,8,3,4,9,7))
...
n <- nrow(fit_binom$L)
x <- rep("other",n)
x[fit_binom$L[,"k8"] > 0.2] <- "loop of henle"
x[fit_binom$L[,"k1"] > 0.2] <- "podocytes"
x[fit_binom$L[,"k3"] > 0.2] <- "endothelial I"
x[fit_binom$L[,"k2"] + fit_binom$L[,"k5"] > 0.2] <- "DCT + CD"
...
set.seed(1)
p2 <- structure_plot(fit_binom,grouping = samples$cluster,gap = 20,
                     colors = topic_colors,topics = c(6,5,2,10,1,8,3,4,9,7),
					 perplexity = 50,verbose = FALSE)
