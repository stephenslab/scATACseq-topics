...
```{r volcano-plots, fig.height=3, fig.width=3}
k <- 8
label_genes <- (is.element(x,loh_genes) & de_gene$logLR[,k] > 4) |
               (de_gene$logLR[,k] > 60 & de_gene$coef[,k] > 0) |
               (de_gene$coef[,k] > 3)
x <- genes
x[!label_genes] <- ""
p8 <- volcano_plot_enrich(de_gene,k = 8,labels = x,
                          highlight = is.element(x,loh_genes),
                          ymax = 200)
```
