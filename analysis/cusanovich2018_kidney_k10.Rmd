...
Here we examine the structure inferred from the topic model fit to
the Mouse sci-ATAC-seq Atlas kidney cells, with $K = 10$ topics.
...
```{r load-de-gene}
load("scripts/de-gene-cusanovich2018-kidney-k=10.RData")
```
...
p1 <- structure_plot(fit_binom_em,grouping = samples$cell_label,gap = 20,
                     verbose = FALSE,colors = topic_colors,
					 topics = c(3,4,6,5,2,1,8,10,9,7))
...
inferred clusters. Instead, we can use the topic proportioins to
subdivide the cells. (Here we are assigning labels to these clusters
only to make it easier to refer to them.)

```{r clusters-from-topics}
n <- nrow(fit_binom_em$L)
x <- rep("other",n)
x[fit_binom_em$L[,"k8"] > 0.2] <- "loop of henle"
x[fit_binom_em$L[,"k1"] > 0.2] <- "podocytes"
x[fit_binom_em$L[,"k3"] > 0.2] <- "endothelial I"
x[fit_binom_em$L[,"k2"] + fit_binom$L[,"k5"] > 0.2] <- "DCT + CD"
samples$cluster <- factor(x,c("DCT + CD","loop of henle","podocytes",
                              "endothelial I","other"))
```
...
set.seed(1)
p2 <- structure_plot(fit_binom_em,grouping = samples$cluster,gap = 20,
                     colors = topic_colors,topics = c(3,4,6,5,2,1,8,10,9,7),
...
These next few volcano plots show, for each gene tested for
enrichment, the mean LFC on the x-axis against the logBF for
enrichment on the y-axis. Marker genes identified in the
[Park *et al* paper](https://doi.org/10.1126/science.aar2131) are
highlighted in red.

```{r volcano-plots-1, fig.height=13, fig.width=8, warning=FALSE}
k <- 2
label_genes <- is.element(genes,cd_genes) |
               (de_gene$logLR[,k] > 35 & de_gene$coef[,k] > 1)
k2_genes <- genes
...
p2 <- volcano_plot_enrich(de_gene,k = 2,labels = k2_genes,
                          ...
                          ymax = 100)
k <- 5
label_genes <- is.element(genes,dct_genes) |
               (de_gene$logLR[,k] > 50 & de_gene$coef[,k] > 1.5) |
			   (de_gene$logLR[,k] > 20 & de_gene$coef[,k] > 2)
k5_genes <- genes
...
label_genes <- (is.element(genes,loh_genes) & de_gene$logLR[,k] > 4) |
               (de_gene$logLR[,k] > 20 & de_gene$coef[,k] > 1)
k8_genes <- genes
...
These next two volcano plots also highlight, in blue, the marker genes
identified in [Wu *et al*](https://doi.org/10.1007/s12079-022-00685-z)
for the S1 and S3 segments of the proximal tubule.

```{r volcano-plots-2, fig.width=8, fig.height=4.5, warning=FALSE}
k <- 9
label_genes <- (is.element(genes,pt_genes) |
               is.element(genes,S1_genes)) &
			   de_gene$logLR[,k] > 10
k9_genes <- genes
...
These volcano plots summarize the gene enrichment results for the
remaining topics.

```{r volcano-plots-3, fig.width=8, fig.height=8.5, warning=FALSE}
k <- 7
label_genes <- (grepl("Zfp",gene_info$Symbol,fixed = TRUE) &
                de_gene$logLR[,k] > 8)
k7_genes <- genes
...
```{r volcano-plots-for-paper, echo=FALSE, warning=FALSE}
ggsave("plots/volcano_plots_cusanovich_kidney_pt.eps",
       plot_grid(p9,p10,nrow = 1,ncol = 2),
	   height = 4.5,width = 8)
ggsave("plots/volcano_plots_cusanovich_kidney_other.eps",
