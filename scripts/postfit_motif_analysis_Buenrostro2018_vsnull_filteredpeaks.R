...
# SETTINGS
# ---------
DAfile          <- '/project2/mstephens/kevinluo/scATACseq-topics/output/Buenrostro_2018/binarized/filtered_peaks/DAanalysis-Buenrostro2018-k=10/DA_regions_topics_vsnull_ash_10000iters.rds'
genome          <- 'hg19'
homerpath       <- '/project2/xinhe/software/homer/bin/findMotifsGenome.pl'
nc              <- 10
out.dir         <- '/project2/mstephens/kevinluo/scATACseq-topics/output/Buenrostro_2018/binarized/filtered_peaks/motifanalysis-Buenrostro2018-k=10-vsnull-ash'
...
# Select regions with DA lfsr < 0.01
# --------------------------------------
homer.dir <- paste0(out.dir, "/HOMER/DA_lfsr_0.01_regions")
cat("Select regions for motif enrichment analysis... \n")
cat(sprintf("%d regions in total. \n", nrow(DA_res$z)))

selected_regions <- select_DA_regions(DA_res, method = "lfsr", thresh.lfsr = 0.01, out.dir = homer.dir, save.bed = TRUE)
